...
themselves. <-- in what way? I can use functions to build the queries. Is that
what was meant?

Bookmarks are now set via a =mu4e-context= and thus exist in my private
configuration.
...
(mu4e-bookmark-define
  (lambda ()
    (concat "maildir:" maildir-account-dir maildir-account-inbox-subdir)
    )
  "inbox"
  ?i)
(mu4e-bookmark-define
