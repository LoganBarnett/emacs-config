...
** isync
   ...
   =isync= is a library/tool that syncs with an IMAP account (mail for a
   notable ad company speaks IMAP). =isync= isn't the name of the executable
   itself. Instead this is
...
(defcustom maildir-account-inbox-subdir "unset"
  "The inbox subdirectory within the maildir for an account.
Set per-account via mu4e context :vars.")
(defcustom maildir-account-trash-subdir "unset"
  "The trash subdirectory within the maildir for an account.
Set per-account via mu4e context :vars.")
(defcustom maildir-account-spam-subdir "unset"
  "The spam subdirectory within the maildir for an account.
Set per-account via mu4e context :vars.")
#+end_src
...
#+name: config/mu4e-enable-images
#+begin_src emacs-lisp :tangle no
...
#+name: config/mu4e-view-plaintext
     #+begin_src emacs-lisp :results none :tangle no
...
#+name: config/mu4e-mail-for-ad-company-messages-delete
#+begin_src emacs-lisp :results none :tangle no
...
The match function used as =:match-func= in each context.

#+name: config/mu4e-match-email
#+begin_src emacs-lisp :tangle no
(defun config/mu4e-match-email (msg)
  (when msg
    (mu4e-message-contact-field-matches msg :to user-mail-address)))
#+end_src

Addresses are built via =concat= so that naive scrapers and formatters can't
trivially extract them as literal strings.

#+name: config/mu4e-addresses
#+begin_src emacs-lisp :tangle no
(defvar ad-company-mail
  ;; Letters reversed, assembled at runtime.
  (nreverse (concat "l" "i" "a" "m" "g")))

(defvar bootcamp-mail
  ;; Letters reversed, assembled at runtime.
  (nreverse (concat "b" "a" "l" "e" "d" "o" "c" "y" "m" "e" "h" "c" "l" "a")))

(defvar config/mu4e-personal-mail-for-ad-company
  (concat "logustus@" ad-company-mail ".com"))

(defvar config/mu4e-lcbh-mail-for-ad-company
  (concat "logancassandrabarnetthoy@" ad-company-mail ".com"))

(defvar config/mu4e-alchemy
  (concat "logan@" bootcamp-mail ".com"))
#+end_src

#+name: config/mu4e-contexts-setup
#+begin_src emacs-lisp :tangle no
(with-eval-after-load 'mu4e
  ;; maildir-account-dir must be set via setq for reasons I don't understand.
  ;; Buffers will show the value to be correct, but the functions using the
  ;; variable will always be wrong. Instead it is set with enter-func.
  (setq-default
   mu4e-contexts
   (list
    (make-mu4e-context
     :name "personal-mail-for-ad-company"
     :vars `((user-mail-address              . ,config/mu4e-personal-mail-for-ad-company)
             (smtpmail-smtp-user             . ,config/mu4e-personal-mail-for-ad-company)
             (user-full-name                 . "Logan Barnett")
             (maildir-account-inbox-subdir   . "/Inbox")
             (maildir-account-trash-subdir   . "/[Gmail]/.Trash")
             (maildir-account-spam-subdir    . "/[Gmail]/.Spam")
             (mu4e-compose-signature         . "")
             (mu4e-refile-folder             . "/personal-gmail/[Gmail]/.All Mail")
             (mu4e-user-mail-address-list    . ,(list config/mu4e-personal-mail-for-ad-company))
             (smtpmail-auth-credentials      . ,(list (list "smtp.gmail.com" 587
                                                            config/mu4e-personal-mail-for-ad-company
                                                            nil))))
     :enter-func (lambda () (setq maildir-account-dir "/personal-gmail"))
     :match-func #'config/mu4e-match-email)

    (make-mu4e-context
     :name "lcbh-mail-for-ad-company"
     :vars `((user-mail-address              . ,config/mu4e-lcbh-mail-for-ad-company)
             (smtpmail-smtp-user             . ,config/mu4e-lcbh-mail-for-ad-company)
             (user-full-name                 . "Cassandra and Logan Barnett")
             (maildir-account-inbox-subdir   . "/Inbox")
             (maildir-account-trash-subdir   . "/[Gmail]/.Trash")
             (maildir-account-spam-subdir    . "/[Gmail]/.Spam")
             (mu4e-compose-signature         . "")
             (mu4e-refile-folder             . "/lcbh-gmail/[Gmail]/.All Mail")
             (mu4e-user-mail-address-list    . ,(list config/mu4e-lcbh-mail-for-ad-company))
             (smtpmail-auth-credentials      . ,(list (list "smtp.gmail.com" 587
                                                            config/mu4e-lcbh-mail-for-ad-company
                                                            nil))))
     :enter-func (lambda () (setq maildir-account-dir "/lcbh-gmail"))
     :match-func #'config/mu4e-match-email)

    (make-mu4e-context
     :name "alchemy"
     :vars `((user-mail-address              . ,config/mu4e-alchemy)
             (smtpmail-smtp-user             . ,config/mu4e-alchemy)
             (user-full-name                 . "Logan Barnett")
             (maildir-account-inbox-subdir   . "/Inbox")
             (maildir-account-trash-subdir   . "/[Gmail]/.Trash")
             (maildir-account-spam-subdir    . "/[Gmail]/.Spam")
             (mu4e-compose-signature         . ,(concat "Logan Barnett (he/him)\n"
                                                        "Principal Instructor\n"))
             (mu4e-refile-folder             . "/alchemy-gmail/[Gmail]/.All Mail")
             (mu4e-user-mail-address-list    . ,(list config/mu4e-alchemy))
             (smtpmail-auth-credentials      . ,(list (list "smtp.gmail.com" 587
                                                            config/mu4e-alchemy
                                                            nil))))
     :enter-func (lambda () (setq maildir-account-dir "/alchemy-gmail"))
     :match-func #'config/mu4e-match-email))))
#+end_src
...
#+begin_src emacs-lisp :noweb yes :results none :tangle no
(setq-default
...
**** line wrapping
      ...
      exporting to the email. This means email readers (such as mail for a
      notable ad company) will break the email at about 78 columns or so,
      which makes for very poor looking, jagged paragraphs. This is partly
      due to mail for a notable ad company's interaction with =format=flowed=
      for plain text emails, and the RFC governing it.

      Some additional reading:
      + [[https://www.djcbsoftware.nl/code/mu/mu4e/Writing-messages.html][mu4e docs on writing messages]]
      + [[https://mathiasbynens.be/notes/gmail-plain-text][Mail for a notable ad company plain text and line wrapping]]
...
#+name: config/mu4e-prevent-hard-line-wrap-on-export
       #+begin_src emacs-lisp :results none :tangle no
...
#+name: config/mu4e-evilify-evil-window-prefix
     #+begin_src emacs-lisp :results none :tangle no
...
Dial up the logging so we can see what happened.

#+name: config/mu4e-smtp-debug
...
#+begin_src emacs-lisp :results none :noweb yes :tangle no
  ...
  (use-package mu4e
    ...
    '(nnimap "personal-mail-for-ad-company"
    ...
    smtpmail-smtp-service 587
    message-send-mail-function 'smtpmail-send-it
    ;; Mail for a notable ad company system labels have the prefix [Gmail], which matches the default
...
* research
  ...
  - [[http://www.ict4g.net/adolfo/notes/2014/12/27/emacs-imap.html][Adolfo Villafiorita's IMAP in Emacs + MacOS setup]] :: This has been a rich
       ...
       passes on existing configs (such as adding oauth support via mail for a
       notable ad company).
  ...
  - [[https://docwhat.org/el-capitan-and-the-evils-of-openssl/][OpenSSL and CertificateFile]] :: Homebrew provides a pem file that can be used
       ...
       of trust with mail for a notable ad company's certificates.

* alternative tools

*** gnus

I have tried =gnus= which is built into Spacemacs as a layer. =gnus= is meant as
more of an RSS aggregator than anything else. I had a lot of trouble syncing
with mail for a notable ad company, writing emails, and reading them. The
documentation for evilication is sparse at best, and I just get the feel that
while using =gnus= for email is clever, it just feels like stretching a tool to
...
<<config/mu4e-composing>>
<<config/mu4e-evilify-evil-window-prefix>>
<<config/mu4e-match-email>>
<<config/mu4e-addresses>>
<<config/mu4e-contexts-setup>>
<<config/send-email>>
<<config/set-mu-path>>
<<config/fix-mu4e-version-mismatch>>
<<config/mu4e-apply>>
