...
#+begin_src emacs-lisp :results none :noweb yes
  (use-package flycheck
    ...
    ;; Enable flycheck in org-mode so the modeline badge shows spelling errors
    ;; via flycheck-aspell (configured below).
  ...
  (use-package flycheck-aspell
    :after flycheck
    :config
    ;; flycheck-aspell registers org-aspell-dynamic automatically on load.
    ;; It uses aspell with a TeX-like filter which strips org markup reasonably
    ;; well (=code=, *bold*, [[links]], etc.).
    ;;
    ;; Inherit the dictionary from ispell so it stays in sync with flyspell.
    (setq flycheck-aspell-language ispell-current-dictionary)
    )
#+end_src
