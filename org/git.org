...
Only enter insert mode if the first line is empty.  This allows for amending
commits or handling pre-populated commit messages while still entering insert
mode for fresh commits (even if pre-commit-msg hooks add text below the first
line).

#+name: config/git-commit-message-evil-insert-state
#+begin_src emacs-lisp :results none :exports code :tangle no
(defun config/git-commit-setup-maybe-insert ()
  "Enter insert mode only if the first line of the commit message is empty."
  (save-excursion
    (goto-char (point-min))
    ...
    (while (and (not (eobp))
                (or (looking-at "^#")
                    (looking-at "^[[:space:]]*$")))
      (forward-line 1))
    ;; If we're at a blank line or end of buffer, enter insert mode.
    (when (or (eobp)
              (looking-at "^[[:space:]]*$"))
      (evil-insert-state))))

(add-hook 'git-commit-setup-hook #'config/git-commit-setup-maybe-insert)
#+end_src

** line numbers

Enable relative line numbers in magit buffers to support vim-style navigation
like ~30j~ and ~3k~.

#+name: config/magit-line-numbers
#+begin_src emacs-lisp :results none :exports code :tangle no
(add-hook 'magit-status-mode-hook #'display-line-numbers-mode)
(add-hook 'magit-diff-mode-hook #'display-line-numbers-mode)
(add-hook 'magit-log-mode-hook #'display-line-numbers-mode)
(add-hook 'magit-revision-mode-hook #'display-line-numbers-mode)
#+end_src


** whitespace visibility in diffs

Show end-of-line =​$= markers in Magit diff and status buffers using a
buffer-local =whitespace-mode= with only the =newline-mark= style.  Trailing
whitespace is intentionally excluded here because Magit's own
=magit-diff-paint-whitespace= covers that — keeping the two concerns separate
avoids double-styling (e.g. red trailing-whitespace face on a green added-line
background).

The =magit-diff-paint-whitespace= family of variables is set in the =use-package
magit= block below so the options are co-located with the rest of the Magit
configuration.

#+name: config/magit-whitespace-setup
#+begin_src emacs-lisp :results none :exports code :tangle no
(defun config/magit-whitespace-setup ()
  "Show EOL markers in Magit buffers without redundant trailing-whitespace faces.
Trailing whitespace is handled separately by `magit-diff-paint-whitespace'."
  (setq-local whitespace-style '(face newline newline-mark))
  (whitespace-mode 1))

(add-hook 'magit-diff-mode-hook   #'config/magit-whitespace-setup)
(add-hook 'magit-status-mode-hook #'config/magit-whitespace-setup)
(add-hook 'magit-revision-mode-hook #'config/magit-whitespace-setup)
#+end_src
...
** syncing git-gutter and magit
  ...
  #+name: config/git-gutter-magit-connect
  #+begin_src emacs-lisp :results none :tangle no
...
This keeps Magit from finding a different window to place the Magit display.

#+name: config/magit-display-buffer-same-window
#+begin_src emacs-lisp :results none :exports code :tangle no
...
(defun +vc/call-with-remote (fn)
  "Call FN with an interactively selected git remote for browse-at-remote.
Temporarily overrides remote resolution so the chosen remote is used
instead of the branch's tracking remote."
  (require 'browse-at-remote)
  (let* ((remote (completing-read "Remote: " (browse-at-remote--get-remotes) nil t))
         (orig (symbol-function 'browse-at-remote--remote-ref)))
    (cl-letf (((symbol-function 'browse-at-remote--remote-ref)
               (lambda (&optional f)
                 (cons remote (cdr (funcall orig f))))))
      (funcall fn))))

(defun +vc/browse-at-remote-for-remote ()
  "Browse file at point on a chosen remote."
  (interactive)
  (+vc/call-with-remote #'browse-at-remote))

(defun +vc/browse-at-remote-kill-for-remote ()
  "Kill file+lines URL for a chosen remote."
  (interactive)
  (+vc/call-with-remote #'browse-at-remote-kill))

(defun +vc/browse-at-remote-kill-homepage-for-remote ()
  "Kill homepage URL for a chosen remote."
  (interactive)
  (+vc/call-with-remote #'+vc/browse-at-remote-kill-homepage))
...
(use-package browse-at-remote
  ...
  (map!
   :leader
   :prefix ("g y M" . "remote url")
   :nv
   :desc "open in browser" "H" #'+vc/browse-at-remote-for-remote
   :desc "kill homepage" "h" #'+vc/browse-at-remote-kill-homepage-for-remote
   :desc "kill lines" "y" #'+vc/browse-at-remote-kill-for-remote
   )
  :config
  ;; Vague hostname matchers for self-hosted forges (e.g., gitea.example.com).
  ;; These prepend to browse-at-remote-remote-type-regexps and take priority
  ;; over the stricter defaults.  Per-repo git-config browseAtRemote.type
  ;; still wins over everything.
  (dolist (entry '((:host "^gitea\\."     :type "gitea")
                   (:host "^gitlab\\."    :type "gitlab")
                   (:host "^github\\."    :type "github")
                   (:host "^bitbucket\\." :type "bitbucket")
                   (:host "^sourcehut\\." :type "sourcehut")))
    (add-to-list 'browse-at-remote-remote-type-regexps entry))
...
<<config/git-gutter-magit-connect>>
;; Not working, and I think in fact is is breaking my local setup.
...
(use-package magit
  ...
  :config
  <<config/magit-display-buffer-same-window>>
  <<config/git-commit-message-evil-insert-state>>
  <<config/magit-line-numbers>>
  <<config/magit-whitespace-setup>>
  ;; Let Magit handle trailing whitespace in diffs natively. `all' includes
  ;; context lines in addition to added/removed lines.
