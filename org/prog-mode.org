...
#+name: config/prog-mode-c-style-set
#+begin_src emacs-lisp :results none :tangle no
...
(use-package indent-bars
  :hook
  ;; indent-bars tries to compute display colors at activation time which
  ;; fails in batch/non-graphical sessions (color-rgb-to-hex gets called on
  ;; "unspecified-bg").  Guard so the mode is only activated when a display
  ;; is present.
  ((prog-mode) . (lambda () (when (display-graphic-p) (indent-bars-mode))))
  :init
...
<<config/auto-fill-comments>>
<<config/prog-mode-c-style-set>>
