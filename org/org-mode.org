...
#+name: config/org-mode-debug-local-variables
#+begin_src emacs-lisp :results none :exports code :tangle no
...
** safe noweb delimiters

I sometimes override the noweb wrap delimiters per-file (via ~Local
Variables:~) so they don't collide with shift operators (=<<= / =>>=) in
languages like C++, Rust, or Java.  The handful of replacements I actually
use are innocuous, so bless them up front to skip the prompt on every load.

A predicate registered via ~put~ is idempotent: re-evaluating the block
overwrites the symbol property with the same (or updated) predicate, and it
composes cleanly with other ~safe-local-variable~ registrations elsewhere
(such as the block in =org/lisp.org=) because each call targets a different
symbol.

#+name: config/org-mode-safe-noweb-delimiters
#+begin_src emacs-lisp :results none :exports code :tangle no
(defun config/org-noweb-wrap-delimiter-safe-p (val)
  "Return non-nil if VAL is a noweb wrap delimiter we have blessed."
  (member val '("<<" ">>" "<<<" ">>>" "<%" "%>" "%<" ">%")))
(put 'org-babel-noweb-wrap-start
     'safe-local-variable
     #'config/org-noweb-wrap-delimiter-safe-p)
(put 'org-babel-noweb-wrap-end
     'safe-local-variable
     #'config/org-noweb-wrap-delimiter-safe-p)
#+end_src
...
#+name: config/iimage-scale-to-fit-width
#+begin_src emacs-lisp :results none :tangle no
...
#+name: config/iimage-scale-on-window-configuration-change
#+begin_src emacs-lisp :results none :tangle no
...
Evil's count-prefixed line motions (~76zz~, ~42gg~, etc.) can land point inside
a folded subtree.  Without intervention org leaves the surrounding content
collapsed, so the cursor appears to "stick" on the parent heading.

Emacs core has no general "cursor jumped here" hook, and ~reveal-mode~ is quirky
with org's fold system.  Evil tags jump motions with a ~:jump~ command property
(it uses this to populate its jumplist), so we can lean on ~post-command-hook~
and let evil classify which commands count as jumps.

We use ~org-fold-show-context~ rather than ~org-fold-show-set-visibility~
because the former only unfolds — the latter normalizes the buffer to a
canonical visibility, which would re-collapse subtrees the user had already
expanded.

#+name: config/org-reveal-after-jump
#+begin_src emacs-lisp :results none :exports code :tangle no
(defun config/org-reveal-after-jump-maybe ()
  "Reveal context around point if an evil jump landed inside a fold."
  (when (and (evil-get-command-property this-command :jump)
             (org-invisible-p))
...
(add-hook 'org-mode-hook
          (lambda ()
            (add-hook 'post-command-hook
                      #'config/org-reveal-after-jump-maybe nil t)))
#+end_src
...
When ~org-babel~ generates a new image (such as from a ~plantuml~ source block),
it will invalidate the "overlay" that displays the image.  To get the image to
display again, we just need to jump on the hook and redisplay the image.

#+name: config/org-babel-images-redisplay-when-recomputed
#+begin_src emacs-lisp :results none :exports code :tangle no
...
Bleh.
#+name: config/ox-html-inline-svgs-fn
#+begin_src emacs-lisp :results none :tangle no
...
#+name: config/org-html-svg-embed
#+begin_src emacs-lisp :results none :tangle no
(require 'ox-html)
(require 'nxml-mode)
...
#+name: config/auto-id-with-confluence-prefix
#+begin_src emacs-lisp :results none :tangle no
...
#+begin_src emacs-lisp :results value list :noweb yes :tangle no
...
#+name: config/org-html-format-headline
#+begin_src emacs-lisp :results none :tangle no
...
(use-package org
  :init
  <<config/org-mode-safe-noweb-delimiters>>
  <<config/org-mode-insert-inactive-time-stamp-with-time-fn>>
  ...
  :config
  <<config/org-babel-images-redisplay-when-recomputed>>
  ...
  (map!
   ...
   (:prefix ("b" . "tables")
    ...
    :desc "align table" "a" #'org-table-align
    :desc "blank field" "b" #'org-table-blank-field
    :desc "create/convert table" "c" #'org-table-create-or-convert-from-region
    ...
    (:prefix ("i" . "insert")
     :desc "insert column" "c" #'org-table-insert-column
     :desc "insert hline" "h" #'org-table-insert-hline
     :desc "insert row" "r" #'org-table-insert-row
     :desc "hline and move" "H" #'org-table-hline-and-move
     )
    (:prefix ("t" . "toggle")
  ...
  <<config/org-html-format-headline>>
  <<config/auto-id-with-confluence-prefix>>
  <<config/org-babel-source-block-languages-fn>>
  <<config/org-time-stamp-string>>
  <<config/org-time-stamp-string-sans-brackets>>
  <<config/clock-full-day>>
  <<config/org-mode-debug-local-variables>>
  (require 'ob-dsq)
