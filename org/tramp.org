...
#+name: config/tramp-shell-settings
#+begin_src emacs-lisp :tangle no
...
* directory variables
  ...
  #+name: config/enable-remote-dir-locals
  #+begin_src emacs-lisp :tangle no
...
#+name: config/fix-tramp-large-file-issue
#+begin_src emacs-lisp :tangle no
...
#+begin_src emacs-lisp :tangle no :results none
;; 1 = quiet.  Verbosity 5 logs and formats *every* remote round-trip into the
;; debug buffer, which is a real per-command tax — magit staging a hunk is
;; dozens of small git commands, so it multiplies.  Bump to 6 via the commented
;; line in lisp/init.el when you actually need to debug a connection.
...
#+name: config/fix-tramp-recentf-truename
#+begin_src emacs-lisp :tangle no :results none
...
#+name: override-tramp-get-ls-command
#+begin_src emacs-lisp :tangle no
...
* git-gutter
  ...
  #+name: config/fix-tramp-git-gutter-interaction
  #+begin_src emacs-lisp :tangle no :results none
...
#+begin_src emacs-lisp :tangle yes :results none :noweb yes
  ...
  <<config/fix-tramp-recentf-truename>>
  ...
  (use-package tramp
    ...
    ;; Defer entirely to ~/.ssh/config's ControlMaster/ControlPath/ControlPersist
    ;; (see nix/home-configs/ssh-config-emacs.nix) so Tramp reuses a *persistent*,
    ;; warm master.  The previous inline value forced ControlPersist=no, which
    ;; tore the master down on every disconnect — the opposite of what we want,
    ;; and it also used a separate socket path from the rest of our ssh usage.
