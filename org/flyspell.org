...
(use-package flyspell
  ...
  ;; Use completing-read as the correction interface so it works with Vertico
  ;; (which enhances completing-read with a popup) and is compatible with
  ;; Embark (which can act on completing-read candidates).
  (setq flyspell-correct-interface
        (lambda (candidates word)
          (let* ((actions '(("Save to personal dictionary" . save)
                            ("Accept for buffer (LocalWords)"  . buffer)
                            ("Accept for session"             . session)
                            ("Skip"                           . skip)))
                 (all-candidates (append candidates (mapcar #'car actions)))
                 (choice (completing-read
                          (format "Correct '%s': " word)
                          all-candidates nil t)))
            (if-let ((action (cdr (assoc choice actions))))
                (cons action word)
              choice))))
