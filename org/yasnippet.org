...
#+begin_src emacs-lisp :results none :tangle no
(setq-default +snippets-dir
  (if (boundp 'config/snippets-dir)
      config/snippets-dir
    (expand-file-name "~/dev/emacs-config/snippets")))
#+end_src
...
* apply
  ...
  (use-package yasnippet
    ...
    ;; Use the Nix-store path when available (set via config/snippets-dir in
    ;; emacs-config-base-dir.el), otherwise fall back to the repo checkout
    ;; location for local development.
    (setq-default yas-snippet-dirs
      (list (if (boundp 'config/snippets-dir)
                config/snippets-dir
              (expand-file-name "~/dev/emacs-config/snippets"))))
    ...
    <<config/yatemplate>>
    (yas-global-mode 1)
    ;; Only recompile and reload snippets if at least one snippet directory
    ;; actually exists.  In a test/CI environment $HOME is a temp directory and
    ;; the dotfiles snippet dir does not exist, which causes yas-recompile-all
    ;; to throw file-missing.  Guard with an existence check so startup still
    ;; succeeds without snippets.
    (when (seq-some (lambda (d) (file-directory-p (expand-file-name d)))
                    yas-snippet-dirs)
      ;; Skip recompilation when snippets live in a read-only directory (e.g.
      ;; the Nix store, indicated by config/snippets-dir being bound).
      ;; yas-recompile-all would fail with permission-denied trying to write
      ;; .yas-compiled-snippets.el files there.  yas-reload-all with NO-JIT=t
      ;; is sufficient to load everything without pre-compilation.
      (unless (boundp 'config/snippets-dir)
        (yas-recompile-all))
