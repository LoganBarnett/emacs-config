;;; init-batteries.el --- My batteries-included settings for Emacs -*- lexical-binding: t; -*-
;;; Commentary:
;;; Entry point into the Emacs config.  The idea to be batteries included - this
;;; gives you a ready-to-go Emacs configuration.
;;; Code:

(defun batteries-init () "Do initializtion." (toggle-debug-on-error)
(message "[INIT] Starting init.")
  ...
  (load-library "ui")
  ;; Backups should not be scattered throughout the directories I'm working in.
  ;; Put them in a place where they will be cleaned out periodically.  The
  ;; documentation says this must be a regular expression, but an expression
  ;; such as `.*' does not work.  So it's... not a regular expression?  Or some
  ;; kind of dialect?  Or perhaps it's Emacs' weird version that has goofy
  ;; things that need escaping.  Who could say for sure?  I postulate that no
  ;; one can.
  ;; If this screws up somehow, you can't use magit's commit, lolz.
  (setq-default backup-directory-alist '(("." . "~/.Trash")))
  ;; Not sure where to put this, since it needs to apply globally.  Perhaps a
  ;; global.org?
  ...
  ;; the heavy tramp-rpc.el defers to first /rpc: access, so it stays inert
  ;; until you opt in per-connection with a /rpc:host: path (/ssh: remains the
  ;; default).  `load' (not `require') avoids a byte-compile-time dependency on
  ;; tramp-rpc.
  ...
  ;; the host's own binary.  These setq's take effect before
  ;; tramp-rpc-deploy.el's defcustoms load, and a defcustom does not overwrite
  ;; an already-set value, so they persist.
  (setq tramp-rpc-deploy-never-deploy t)
  (setq tramp-rpc-deploy-remote-binary-path
  ...
  ;; turned me off to the whole thing, and now I'm shelving this work entirely.
...
(defvar config/init-complete-hook
  nil'
